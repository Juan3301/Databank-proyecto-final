...
from services import bonus_admin
...
def load_bank():
    ...
    if os.path.exists(data_file):
        try:
            bank.import_data_json(data_file)
        except Exception as e:
            print(f"No se pudo importar datos previos: {e}")

    return bank

bank = load_bank()
bonus_admin = bank.bonus_admin


if not bank.employees:
    director = Director("Laura Espinosa", "1021678463", "Ingeniería", 1, "julilaura")
    bank.manage_employees.add_employee(director, director)
else:
    director = next(
        (e for e in bank.employees if isinstance(e, Director)),
        None
    )

    if director is None:
        raise RuntimeError("No se encontró un Director en los empleados cargados desde el JSON.")
...
account_class_to_label = {v: k for k, v in account_type_to_class.items()}

def get_account_label(account):
    class_name = type(account).__name__
    return account_class_to_label.get(class_name, class_name)
...
def go_to_register_client_screen(username, bank):
    ...
    def save_new_client():
        ...
        if not dni_text.isdigit():
            label_error.config(text="El DNI solo puede contener números.")
            return

        if len(dni_text) < 6 or len(dni_text) > 10:
            label_error.config(text="El DNI debe tener entre 6 y 10 dígitos.")
            return
        ...
        try:
            ...
            if dni <= 0:
                label_error.config(text="El DNI debe ser un número positivo.")
                return

            if age <= 0 or age > 120:
                label_error.config(text="Ingrese una edad válida.")

            if age < 13:
                label_error.config(text="Debes tener al menos 13 años para registrarte en el banco.")
                return
    ...
    tk.Button(
        ...
        command=lambda: go_to_main_menu(username)
    ).pack(pady=5)
...
def go_to_client_operations(client_object, bank):
    ...
    if not accounts:
        ...
        for c in accounts:
            ...
            tk.Label(
                frame_accounts, 
                text=f"Cuenta {get_account_label(c)} -> Saldo: ${c.get_balance()}", 
                font=("Courier", 11), 
                fg="white", 
                bg="#00296b"
...
def open_account_module(client_object, bank):
    ...
    def label_for(account):
        ...
        return f"{get_account_label(account)} - ****{number[-4:]}"
...
def open_transfer_module(account, client_object, bank):
    ...
    def transfer_gui():
        ...
        if len(password_text) != 4 or not password_text.isdigit():
            error_label.config(text="El PIN debe contener exactamente 4 dígitos.")
            return
        ...
        if not destination_text.isdigit():
            error_label.config(text="Ingrese un número de cuenta válido.")
            return
        ...
        if destination_text == str(account.account_number):
        ...
        if associated_card.is_blocked:
            error_label.config(text="La tarjeta está bloqueada, no puede realizar transferencias.")
            return
        ...
        except ImpossibleOperationException as e:
            error_label.config(text=str(e))
...
def open_credits_module(client_object, bank):
    ...
    def request_credit_gui():
        ...
        try:
            credit = bank.manage_credits.request_credit(
                client_object,
                amount,
                int(selected_months.get())
            )
            ...
            client_object.requests_history.append(
                f"[{actual}] Solicitud de crédito por ${amount:,.2f} "
                f"a {selected_months.get()} meses. Estado: Pendiente."
            )
            bank.export_data_json()
            ...
            error_label.config(text=f"Error solicitando crédito: {e}")
...
def open_cards_module(client_object, bank):
    ...
    def process_blocking(client_object, bank, card_obj):
        if card_obj:
            try:
                ...
                card_obj.blocked_at = datetime.now()
    ...
    def process_unblocking(client_object, bank, card_obj):
            if not hasattr(card_obj, "blocked_at"):
                show_success_screen(
                    client_object,
                    bank,
                    "No se puede desbloquear",
                    "No hay registro del bloqueo."
                )
                return

            elapsed = datetime.now() - card_obj.blocked_at
            ...
            if elapsed < timedelta(minutes=30):
                ...
                show_success_screen(
                    ...
                    f"Debes esperar {minutes} minutos más."
                )
                return

            try:
                bank.manage_cards.unblock_card(card_obj)
                card_obj.is_blocked = False

                show_success_screen(
                    client_object,
                    bank,
                    "Tarjeta desbloqueada",
                    "Tu tarjeta vuelve a estar activa."
                )
    ...
    if real_card and getattr(real_card, "is_blocked", False):

        card_button_text = "Desbloquear tarjeta"
        card_button_command = lambda: process_unblocking(client_object, bank, real_card)

    else:

        card_button_text = "Bloquear tarjeta"
        card_button_command = lambda: process_blocking(client_object, bank, real_card)


    tk.Button(
        actions_inner_frame,
        text=card_button_text,
        command=card_button_command,
        **action_button_style
    ).pack(pady=15, ipady=8)
    ...
    faq_list = [
        {
            ...
            "answer": "Para abrir una cuenta, ingresa tu nombre en la pantalla principal, selecciona 'Crear nueva cuenta', completa tus datos personales y autoriza el tratamiento de datos. Sin embargo, si deseas añadir una nueva, ve a la sección cuentas."
        },
        {
...
def go_to_employee_login_screen(username,bank):
    ...
    entry_dni.focus() #hace que el cursor aparezca listo para escribir
...
def go_to_register_employee_screen(dni, bank):
    ...
    def save_new_employee(dni):
        ...
        try:
            ...
            if experience < 0 or experience > 85:
                label_error.config(text="Los años de experiencia deben estar entre 0 y 85.")
                return
    ...
    tk.Button(
        ...
        bg="white",
        fg="#00296b",
        command= lambda : go_to_employee_login_screen(dni, bank),
        relief="flat",
        activebackground="#9C9C9C",
        activeforeground="white"
    ).pack(pady=20, ipadx=10, ipady=5)
...
def open_faq_e_module(employee, bank):
    ...
    faq_list = [
        ...
        {
            "question": "¿Puedo tener una cuenta bancaria si soy empleado?",
            "answer": "Así es, cierra sesión, dirígete a la sección cliente e ingresa el mismo dni que tienes registrado como empleado."
        },
        {
...
def raise_salaries_module(director, bank):
    ...
    if not requests_list:
        ...
        for req in requests_list:
            ...
            info_text = (
                f"Empleado: {req_employee.name} (DNI: {req_employee.get_dni()})\n"
    ...
    tk.Button(
        ...
        activebackground="#9C9C9C",
        activeforeground="white",
        command=lambda: render_director_panel(director, bank)
...
def accept_salary_increase(director, bank, request, target_employee):
    try:
        ...
        messagebox.showinfo(
            "Aumento aprobado",
            "El aumento salarial fue aprobado correctamente."
        )
...
def reject_salary_increase(director, bank, request):
    ...
    log_entry = AuditLog(
        ...
        target_dni=request.employee.get_dni(),
...
def create_add_users(employee, bank):
    ...
    tk.Button(
        ...
        activebackground="#9C9C9C",
        activeforeground="white",
        command=lambda: render_director_panel(employee, bank)
    ).pack(pady=20, ipadx=20, ipady=5)
...
def open_get_dni_screen(employee, bank, action_type):
    ...
    def process_dni():
        ...
        if action_type == "create":
            go_to_add_employee_screen(dni_text, bank, operator=employee)
        
        elif action_type == "delete":
            target_employee = bank.search.search_employee_by_dni(dni_text)
...
def open_e_account_module(employee_role_obj, bank):
    ...
    user_data_entries = [
        ...
        ("Documento (DNI):", str(employee_role_obj.get_dni())),
...
def accept_promotion(director, bank, request, target_employee):
    try:
        ...
        bank.audit_logs.append(log_entry)
...
def render_bank_panel(employee, bank):
    ...
    buttons_config = [
        ...
        ("\nRegistrar\nLog Manual", lambda: open_register_log_module(employee, bank), lock_img, 1, 1),
...
def open_global_tx_module(employee, bank):
    ...
    transactions = bank.get_global_transactions(employee)
...
def open_account_history_module(employee, bank):
    ...
    label_error = tk.Label(search_frame, text="", font=("Courier", 10, "bold"), fg="red", bg="#00296b")
    label_error.pack(side="left", padx=5)
    ...
    def search_history():
        ...
        if not acc_id:
            ...
            account = next((a for a in bank.accounts if str(a.account_number) == acc_id), None)

            if not account:
                label_error.config(text="No se ha encontrado cuenta.")
            else:
...
def open_reports_selector_module(employee, bank):
    ...
    financial_labels = {
        ...
        "total_money": "Dinero total en el banco",
        }
        ...
        if output:
            if isinstance(output, dict):
                for key, value in output.items():
                    ...
                    if key == "total_money":
                        report_display.insert(tk.END, f"{label}: ${value:,.2f}\n")
                    else:
                        report_display.insert(tk.END, f"{label}: {value}\n")

            elif isinstance(output, (list, tuple)):
                for item in output:
                    report_display.insert(tk.END, str(item).replace("\\n", "\n") + "\n\n")
            else:
                report_display.insert(tk.END, str(output).replace("\\n", "\n"))
        else:
